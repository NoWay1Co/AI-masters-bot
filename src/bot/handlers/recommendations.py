...
async def get_recommendations(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text("Генерирую персональные рекомендации... Пожалуйста, подождите.")
        ...
        # Генерируем персональные рекомендации через LLM
        recommendation = await _generate_personalized_recommendations_llm(user_profile, programs)
        
        if not recommendation:
            # Fallback рекомендации если LLM недоступна
...
async def _generate_personalized_recommendations_llm(user_profile, programs):
    """Генерирует персональные рекомендации через LLM с точки зрения выбора"""
    from ...services.llm_service import llm_service
    
    # Формируем контекст с профилем пользователя и всеми данными о программах
    user_context = f"""
Профиль пользователя:
- Образование/опыт: {user_profile.background or 'Не указано'}
- Интересы: {', '.join(user_profile.interests) if user_profile.interests else 'Не указаны'}
- Цели обучения: {', '.join(user_profile.goals) if user_profile.goals else 'Не указаны'}
"""
    
    # Формируем полную информацию о программах и курсах
    programs_context = "Доступные магистерские программы ИТМО:\n\n"
    for program in programs:
        programs_context += f"ПРОГРАММА: {program.name}\n"
        programs_context += f"Описание: {program.description or 'Инновационная программа по ИИ'}\n"
        programs_context += f"Общие кредиты: {program.total_credits}\n"
        programs_context += f"Всего курсов: {len(program.courses)}\n\n"
        
        # Добавляем обязательные курсы
        mandatory_courses = [c for c in program.courses if not c.is_elective]
        if mandatory_courses:
            programs_context += "ОБЯЗАТЕЛЬНЫЕ КУРСЫ:\n"
            for course in mandatory_courses:
                programs_context += f"- {course.name} ({course.credits} кредитов, {course.semester} семестр)\n"
            programs_context += "\n"
        
        # Добавляем выборочные курсы
        elective_courses = [c for c in program.courses if c.is_elective]
        if elective_courses:
            programs_context += "ВЫБОРОЧНЫЕ КУРСЫ:\n"
    ...
    prompt = f"""
Ты - опытный консультант по образованию, который сам прошел обучение в сфере ИИ. 

{user_context}

{programs_context}

ЗАДАЧА: На основе профиля пользователя выбери программу, на которую бы ты сам пошел, будучи на его месте, и объясни почему именно эта программа подходит под его профиль.

ВАЖНЫЕ ТРЕБОВАНИЯ:
Говори от первого лица - "Я бы выбрал эту программу, потому что..."
Обоснуй выбор конкретными фактами из профиля
Порекомендуй 3-5 конкретных курсов из программы под интересы
Укажи карьерные возможности после окончания
Дай практические советы по подготовке

СТРУКТУРА ОТВЕТА:
1. Персональная рекомендация программы (с обоснованием)
2. Рекомендуемые курсы под интересы
3. Карьерные перспективы
4. Практические советы

СТИЛЬ: Личный, экспертный, с конкретными рекомендациями. Используй эмодзи.
...
"""
    ...
    recommendations += """
*СЛЕДУЮЩИЕ ШАГИ:*
• Изучите детали программ → "Выбрать программу"
• Сравните программы между собой
• Скачайте полную информацию о курсах
• Задайте вопросы в режиме Q&A
